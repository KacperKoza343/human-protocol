...
describe('AuthService E2E', () => {
  ...
  it('should test authentication workflow', async () => {
    ...
    // User Registration
    const signUpResponse = await request(BASE_URL).post('/auth/signup').send({
      email,
      password,
    ...
    const invalidVerifyTokenResponse = await request(BASE_URL)
    ...
    // Valid verification
    const verifyTokenResponse = await request(BASE_URL)
      .post(`/auth/email-verification`)
    ...
    const refreshTokenEntity = await tokenRepository.findOneByUserIdAndType(
      refreshedUserEntity?.id as number,
      TokenType.REFRESH,
    );
    expect(refreshTokenEntity).toBeDefined();

    // Invalid Authentication
    const invalidSignInResponse = await request(BASE_URL)
      .post('/auth/signin')
      .send({
        email,
        password: 'Incorrect',
    ...
    // Refresh Token
    const refreshTokenResponse = await request(BASE_URL)
      .post('/auth/refresh')
      .send({ refresh_token: refreshTokenEntity?.uuid });
    ...
    const resendEmailVerificationResponse = await request(BASE_URL)
