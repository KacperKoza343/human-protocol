...
export class WebhookService {
  ...
  public async processPendingWebhook(
    ...
    try {
      ...
      const manifest: ManifestDto | CampaignManifestDto = await StorageClient.downloadFileFromUrl(
        manifestUrl,
      );
      ...
      let finalResults: FortuneFinalResult[] | ImageLabelBinaryJobResults | CampaignFinalResult[] = [];
      ...
      if (
        ...
        manifest.requestType === JobRequestType.CAMPAIGN &&
        intermediateResults instanceof CampaignFinalResult
      ){
        finalResults = intermediateResults;
        // checkPassed = intermediateResults.length <= finalResults.length;
      }
      ...
      if (
        manifest.requestType === JobRequestType.CAMPAIGN &&
        finalResults instanceof Array<CampaignFinalResult>
      ){
        recipients = this.getRecipients(finalResults, manifest.requestType);
        const results = finalResults as Array<CampaignFinalResult>
        amounts = this.calculateCampaignPayoutAmounts(
          manifest.fundAmount,
          recipients,
          results
        );
      ...
      recipients = this.getRecipients(finalResults, manifest.requestType);
      amounts = this.calculatePayoutAmounts(
        manifest.fundAmount,
        recipients.length,
      );
    }
  ...
  public calculateCampaignPayoutAmounts(totalAmount: string, recipient:Array<string>, results:CampaignFinalResult[]) :BigNumber[]{
    ...
    const payouts = bigNumberResults.map(result => {
      ...
      return BigNumber.from(totalAmount).mul(participantPercentage);
    });
  ...
  public getRecipients(
    finalResults: FortuneFinalResult[] | ImageLabelBinaryJobResults | CampaignFinalResult[],
    ...
    ) 
    {
      ...
      const results = finalResults as CampaignFinalResult[];

      return results.map(
        (item) => item.workerIdentifier,
      );
    }
